/*!
*   Optocoupled inputs usage example for Sequent Microsystems Home Automation HAT
...
*
*For this example to work as intended you have to connect the loopback cable between relays and optocoupled inputs.	
*	
*
*/
 ...
 if (card.isAlive() && relay_loops < 3)// limit loops to 3 due to the relays noise
  {
    ...
    for (int i = 1; i < 9; i++)
    {
      card.writeRelay(i, 1);
      delay(250);
	  Serial.print( "opto bitmap value: ");
	  Serial.println(card.readOpto(), BIN);
    }
    for (int i = 1; i < 9; i++)
    {
      card.writeRelay(i, 0);
